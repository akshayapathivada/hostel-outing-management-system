...
* {
    ...
    <center><h1>UPDATE STUDENT IN STATUS</h1></center>
    <div class="container" style="margin-top:50px">
        <form method="GET" action="SearchDelete1.jsp">
...
<button type="submit" value="view" style="border-radius:5px;background-color:rgb(64, 241, 64);width:50px;height:35px">Search</button></center>
    ...
    if (rollNumber != null && !rollNumber.isEmpty() && !rollNumber.equals("null") && !rollNumber.equals("") && !(request.getParameter("reset") != null && request.getParameter("reset").equals("true"))) {
           try {
               ...
               PreparedStatement searchStm = con.prepareStatement("SELECT * FROM outings WHERE redg = ? and status='OUT' ");
               ...
               if (rs.next()) {
                   %>
                   <form method="POST" action="SearchDelete1.jsp">
                        <div class="child" id="c1"><input type="hidden" name="rollNumber" value="<%= rollNumber %>" /><br><%= rollNumber %>
                            <button class="button" type="submit" name="action" value="delete" style="margin-left:500px">IN</button>
                            </div>
                   </form>
                   <%
               } 
               else {
                out.println("<p>Already Inside the College.</p>");
            }
               con.close();
           } catch (Exception e) {
               e.printStackTrace();
    ...
    <% String action = request.getParameter("action");
       if (action != null && action.equals("delete")) {
           String rollNumberToDelete = request.getParameter("rollNumber");
           if (rollNumberToDelete != null && !rollNumberToDelete.isEmpty()) {
               try {
                   Class.forName("com.mysql.jdbc.Driver");
                   ...
                   PreparedStatement deleteStm = con.prepareStatement("delete from outings WHERE redg = ?");
                   deleteStm.setString(1, rollNumberToDelete);
                   int rowsAffected = deleteStm.executeUpdate();

                   if (rowsAffected > 0) {
                    out.println("<p>Updated Status Successfully</p>");
                       response.sendRedirect("SearchDelete1.jsp?reset=true");
                   } else {
